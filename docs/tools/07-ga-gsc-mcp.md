...
**Depends on:** `@aeo/mcp-core`, `@aeo/google-api`, `@aeo/types`
...
## Server
- `@aeo/mcp-core` `createServer` + `registerTool` (zod schemas), `RateLimiter`.
...
- Reuses `@aeo/google-api` `Ga4Client` / `GscClient` / `GoogleOAuth` — no new Google code.
