...
- the blogging agent runs as a **Vercel Cron** (`/api/cron/blogging`), its orchestration lifted into a
  shared `@aeo/blogging` package.

The standalone apps were deleted; their logic was preserved in the `@aeo/*` packages, so this was
...
`@aeo/storage` already targets `@supabase/supabase-js`, so it needs **no code change**.

## Consequences
- One project, one domain, one env set; a natural home for the future auth + billing layer.
- The shared `@aeo/*` engines made consolidation cheap — the apps were always thin wrappers.
