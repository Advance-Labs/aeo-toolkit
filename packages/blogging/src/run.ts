 ...
 * `runBloggingPipeline()` builds the production wiring from the environment and invokes it.
 */
...
/**
 * Top-level entry: build production dependencies from the environment and run one pipeline pass.
 *
 * This is the env-driven wiring the deployment (Vercel Cron) invokes. The DI core is
 * {@link runPipeline}; this function resolves all seams from `env` and delegates to it.
 *
 * Wiring choices:
 ...
 *   - Strategy: seeded from `CONTENT_PILLARS`/`COMPETITORS`/`AUDIENCE`/`VOICE`, or a minimal default.
 *
 * BYOK keys and Google credentials are read from `env` at call time, live only in the resolved
 * config for the duration of the run, and are never persisted or logged. The returned
 * {@link RunSummary} is log-safe (contains no secrets).
 */
export async function runBloggingPipeline(env: Env = process.env): Promise<RunSummary> {
