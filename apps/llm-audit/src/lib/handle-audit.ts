/**
 * Core audit request handler, kept OUT of the route file so the rate limiter can be injected for
 * tests (no Redis, no shared module state) — Next.js route modules may only export route handlers,
 * so a testable helper has to live here in `lib/`.
 */
import { NextResponse } from 'next/server';
import type { AuditReport } from '@aeo/types';
import { runAudit } from './audit-pipeline.js';
import { toErrorBody } from './errors.js';
import { parseAuditRequest } from './validate.js';
