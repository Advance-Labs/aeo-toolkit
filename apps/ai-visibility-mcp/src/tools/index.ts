/**
 * Tool registry — wires each tool's zod input schema + handler onto a
 * `CreatedServer` via `@aeo/mcp-core#registerTool`. Handlers call the pure
 * logic in `logic.ts`, then shape the result into an MCP `ToolResult` carrying
 * both a human-readable text block and machine-readable `structuredContent`.
 *
 * The dependency seam (`ToolDeps`) is closed over here so both transports
 * (stdio + HTTP) register the identical tool set.
 */
import { z } from 'zod';
import { registerTool, type CreatedServer, type ToolResult } from '@aeo/mcp-core';
...
/**
 * Register all five visibility tools. `deps` defaults to the real `@aeo/*`
 * functions; tests pass a fake `ToolDeps`.
 */
export function registerAllTools(server: CreatedServer, deps: ToolDeps = defaultDeps): void {
  registerTool(server, {
