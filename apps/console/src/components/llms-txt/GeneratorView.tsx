 ...
 * app. The shell layout provides the page `<main>`, so this renders inside a plain
 * wrapper (no nested `<main>`). Calls `POST /api/generate`.
 */
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-8">
      <header className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold tracking-tight text-white">llms.txt Generator</h1>
      ...
      {result ? (
        <div className="flex flex-col gap-6">
          <p className="text-sm text-slate-400">
            Crawled <span className="font-medium text-slate-300">{result.url}</span> —{' '}
            {result.pageCount} page{result.pageCount === 1 ? '' : 's'} included.
          </p>
          {files.map((file) => (
