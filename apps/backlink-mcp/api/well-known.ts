/**
 * Vercel Function entry for OAuth 2.1 `.well-known` discovery.
 *
 * Claude.ai fetches these to auto-register against the hosted MCP server. The
 * documents are pure JSON built from env (`MCP_PUBLIC_URL` / `BACKLINK_PUBLIC_URL`,
 * `BACKLINK_AUTH_SERVER`) via the shared builders in `src/http.ts`, so the hosted
 * and self-run variants return identical metadata.
 *
